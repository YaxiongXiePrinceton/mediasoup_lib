...
namespace RTC
{
	  ...
	  : id(id), listener(listener)
	{
		...
		auto jsonKindIt = data.find("kind");
        	MS_DUMP("Yaxiong: Producer::Producer() ");
		...
		for (auto& exten : this->rtpParameters.headerExtensions)
		{
			...
			{
                MS_DUMP("Yaxiong: We are setting the transportWideCc01 id: %d\n", exten.id);
            ...
            //yaxiong 
            //
		    if (this->rtpHeaderExtensionIds.capacityFeedback == 0u && exten.type == RTC::RtpHeaderExtensionUri::Type::CAPACITY_FEEDBACK)
			{
                MS_DUMP("Yaxiong: We are setting the capacity feedback id: %d\n", exten.id);
				this->rtpHeaderExtensionIds.capacityFeedback = exten.id;
			}
	...
	inline bool Producer::MangleRtpPacket(RTC::RtpPacket* packet, RTC::RtpStreamRecv* rtpStream) const
	{
		...
		// Mangle RTP header extensions.
		{
			...
			else if (this->kind == RTC::Media::Kind::VIDEO)
			{
                ...
                // Proxy urn:ietf:params:rtp-hdrext:toffset.
				extenValue = packet->GetExtension(this->rtpHeaderExtensionIds.capacityFeedback, extenLen);
				...
				if (extenValue)
				{
					...
					extensions.emplace_back(
					  static_cast<uint8_t>(RTC::RtpHeaderExtensionUri::Type::CAPACITY_FEEDBACK), extenLen, bufferPtr);
            ...
            //yaxiong
           	packet->SetCapacityFeedbackExtensionId(
			  static_cast<uint8_t>(RTC::RtpHeaderExtensionUri::Type::CAPACITY_FEEDBACK));
