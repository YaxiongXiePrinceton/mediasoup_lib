...
namespace RTC
{
	...
	void RtpPacket::SetExtensions(uint8_t type, const std::vector<GenericExtension>& extensions)
	{
		...
		this->capacityFeedbackExtensionId  = 0u;
	...
	RtpPacket* RtpPacket::Clone(const uint8_t* buffer) const
	{
		...
		packet->capacityFeedbackExtensionId  = this->capacityFeedbackExtensionId;
...
//                if(id == 8){
//                    uint32_t  val = Utils::Byte::Get3Bytes(ptr+1, 0);
//				    printf(" RtpPacket::ParseExtensions id: %d len:%d val:%d\n", id, len, val);
//                }
				//printf(" RtpPacket::ParseExtensions id: %d len:%d \n", id, len);
