...
namespace RTC
{
	...
	public:
		...
		void SetAbsSendTimeExtensionId(uint8_t id)
		{
        	...
        	void SetCapacityFeedbackExtensionId(uint8_t id)
		{
			this->capacityFeedbackExtensionId = id;
		}
        ...
        bool ReadCapacityFeedback(uint32_t& capacityFeedback) const
		{
			uint8_t extenLen;
			uint8_t* extenValue = GetExtension(this->capacityFeedbackExtensionId, extenLen);

			if (!extenValue || extenLen != 3u)
				return false;

			capacityFeedback = Utils::Byte::Get3Bytes(extenValue, 0);
	...
	private:
		...
		uint8_t capacityFeedbackExtensionId{ 0u };
